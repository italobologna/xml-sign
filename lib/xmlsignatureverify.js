...
async function verifyReferencedNode(referencedNode, ref) {
  ...
  const calculatedNodeDigest = crypto.createHash('SHA256')
      .update(handledNode)
      .digest('base64');
  const refDigest = ref.getElementsByTagName(
      'DigestValue')[0]?.childNodes[0]?.toString();
...
module.exports = async function verifyXmlSignature(xml, certPem) {
  ...
  for (const ref of Array.from(references)) {
    const path = getReferencedNode(ref);
  ...
  let signatureValue = signatureNode
      .getElementsByTagName('SignatureValue')[0]?.childNodes[0]?.toString();
